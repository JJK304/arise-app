...
export default function AriseApp() {
  ...
  return (
    <div style={{ minHeight:"100dvh",background:"#050508",fontFamily:"'Rajdhani',sans-serif",color:"#e2e8f0",backgroundImage:`radial-gradient(ellipse at 50% -5%,${rc.glow},transparent 55%)`,maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column",position:"relative",paddingTop:"env(safe-area-inset-top)",paddingLeft:"env(safe-area-inset-left)",paddingRight:"env(safe-area-inset-right)" }}>
      <link href="https://fonts.googleapis.com/css2?family=Rajdhani:wght@400;600;700&family=Orbitron:wght@700;900&display=swap" rel="stylesheet"/>
      <style>{`@keyframes fadeInOut{0%{opacity:0;transform:scale(.85)}15%{opacity:1;transform:scale(1)}80%{opacity:1}100%{opacity:0}} @keyframes slideDown{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:translateY(0)}} @keyframes glitch{0%,100%{transform:translate(0)}25%{transform:translate(-2px,1px)}75%{transform:translate(2px,-1px)}} *{-webkit-tap-highlight-color:transparent;} input::placeholder{color:#2d3748;} html,body{background:#050508 !important;} @supports(padding:max(0px)){.safe-bottom{padding-bottom:max(18px,env(safe-area-inset-bottom))}}`}</style>
      ...
      {/* Header */}
      <div style={{ padding:"16px 18px 13px",borderBottom:"1px solid rgba(255,255,255,0.04)",background:"rgba(0,0,0,0.55)",backdropFilter:"blur(14px)",position:"sticky",top:0,zIndex:100 }}>
        ...
        <div>
          ...
          {view==="quests" && <div style={{ fontSize:"0.56rem",color:"#1e293b",marginTop:3 }}>Heute: {todayDone}/{currentDB.daily.length} tägl. · {totalMilestonesDone} Meilensteine</div>}
        </div>
      </div>
      ...
      <div style={{ flex:1,overflowY:"auto",padding:"15px 13px 110px" }}>
        ...
        {view==="profile" && (
          <div>
            ...
            {/* Stats quick view */}
            <div style={{ fontSize:"0.56rem",letterSpacing:"0.28em",color:"#1e293b",marginBottom:9 }}>KERN-STATS</div>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
              ...
              {displayChallenges.map(c=>(
                <div key={c.id} style={{ position:"relative" }}>
                  ...
                  {c.type==="custom" && !state.completedChallenges?.includes(c.id) && (
                    <button onClick={()=>deleteCustomQuest(c.id)} style={{ position:"absolute",top:8,right:8,background:"transparent",border:"none",color:"#334155",fontSize:"0.8rem",cursor:"pointer",padding:4 }}>✕</button>
                  )}
                </div>
              ))}
            </div>
                ...
                <div key={sc.key}>
                  ...
                  {sc.sub && (
                    <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,marginTop:5,marginLeft:10 }}>
                      {sc.sub.map(k=><StatBar key={k} label={k} icon={SUB_STATS[k].icon} value={state.stats[k]||0} max={Math.max(10,(state.stats[k]||0)*1.8+5)} color={SUB_STATS[k].color} small/>)}
                    </div>
                  )}
                </div>
              ))}
        ...
        {view==="more" && (
          <div>
              ...
              {ACHIEVEMENTS.map(a=>{
                ...
                return (
                  <div key={a.id} style={{ background:unlocked?"rgba(245,158,11,0.06)":"rgba(255,255,255,0.015)",border:`1px solid ${unlocked?"#f59e0b33":"#0a0a14"}`,borderRadius:9,padding:"10px 13px",display:"flex",alignItems:"center",gap:12,opacity:unlocked?1:0.4 }}>
                    ...
                    {unlocked && <span style={{ marginLeft:"auto",color:"#22c55e",fontSize:"0.9rem",flexShrink:0 }}>✓</span>}
                  </div>
                );
              })}
            </div>
            ...
            {/* Backup / Restore */}
            <div style={{ fontSize:"0.56rem",letterSpacing:"0.28em",color:"#1e293b",marginBottom:11 }}>DATEN</div>
            <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
              ...
              <div style={{ marginTop:8 }}>
                {!confirmReset ? (
                  <button onClick={()=>setConfirmReset(true)} style={{ width:"100%",background:"rgba(239,68,68,0.07)",border:"1px solid #ef444422",color:"#ef4444",borderRadius:10,padding:"13px",fontSize:"0.82rem",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,cursor:"pointer",letterSpacing:"0.08em",display:"flex",alignItems:"center",justifyContent:"center",gap:8 }}>
                    ⚠ RESET — ALLES LÖSCHEN
                  </button>
                ) : (
                  <div style={{ background:"rgba(239,68,68,0.08)",border:"1px solid #ef444444",borderRadius:10,padding:"14px" }}>
                    ...
                    <div style={{ color:"#7f1d1d",fontSize:"0.72rem",marginBottom:12,lineHeight:1.5 }}>Rang, Level, XP, Stats, Körper-Daten — alles wird auf 0 zurückgesetzt. Dein Backup bleibt erhalten wenn du vorher eines heruntergeladen hast.</div>
                    <div style={{ display:"flex",gap:8 }}>
                      <button onClick={handleReset} style={{ flex:1,background:"linear-gradient(135deg,#ef444418,#ef444430)",border:"1px solid #ef444466",color:"#ef4444",borderRadius:8,padding:"11px",fontSize:"0.82rem",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,cursor:"pointer",letterSpacing:"0.06em" }}>
                        JA, LÖSCHEN
                      </button>
                      <button onClick={()=>setConfirmReset(false)} style={{ flex:1,background:"transparent",border:"1px solid #1a1a2e",color:"#334155",borderRadius:8,padding:"11px",fontSize:"0.82rem",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,cursor:"pointer" }}>
                        ABBRECHEN
                      </button>
                    </div>
                  </div>
      ...
      {/* BOTTOM NAV */}
      <div style={{ position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:"rgba(2,2,5,0.97)",borderTop:"1px solid rgba(255,255,255,0.04)",backdropFilter:"blur(20px)",display:"flex",padding:`8px 0 calc(18px + env(safe-area-inset-bottom))`,zIndex:200 }}>
        {navItems.map(item=>{
          ...
          return (
            <button key={item.id} onClick={()=>setView(item.id)} style={{ flex:1,background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"7px 0",transition:"all 0.2s" }}>
              <span style={{ fontSize:"0.9rem",color:active?rc.primary:"#1a1a28",textShadow:active?`0 0 8px ${rc.primary}`:"none",transition:"all 0.2s" }}>{item.icon}</span>
              <span style={{ fontSize:"0.52rem",letterSpacing:"0.1em",color:active?rc.primary:"#1a1a28",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,transition:"all 0.2s" }}>{item.label}</span>
            </button>
          );
        })}
      </div>
