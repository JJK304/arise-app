...
export default function AriseApp() {
  ...
  const [bodyEntries, setBodyEntries] = useState(() => LS("arise_body") || []);
  ...
  // Load
  useEffect(() => { const s = LS("arise_v3"); if(s) setState(s); }, []);
  ...
  useEffect(() => {
    ...
    if(changed) { setState(u); LS("arise_v3", u); }
  ...
  const saveBodyEntry = () => {
    ...
    setBodyEntries(updated); LS("arise_body", updated);
  ...
  const importData = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if(data.state) { LS("arise_v3", data.state); setState(data.state); }
        if(data.body)  { LS("arise_body", data.body); setBodyEntries(data.body); }
